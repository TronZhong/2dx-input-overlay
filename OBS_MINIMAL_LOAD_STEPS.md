...
- Windows 10/11 x64
- Visual Studio 2026 或 Visual Studio 2022（含 C++ 桌面工具链）
...
成功判定：
- 末尾出现 `[OK] OBS plugin build completed via preset ...`。
- 脚本会优先尝试 `windows-x64`，失败后自动回退到 `windows-vs2022-x64`。
- 产物统一安装到 `obs-plugintemplate/release/`。
...
## 4. 本机直装到 OBS 目录（调试，可选）

如果只做本机联调，可在 `run_build.cmd` 成功后，把 `release/` 下对应插件目录覆盖到 OBS 插件目录。

如需走 `cmake --install` 直装方式，注意：
- 配置应与当前脚本一致（`Release`）。
- 构建目录取决于实际使用的 preset（`build_x64` 或 `build_vs2022_x64`）。
...
- 生成器不匹配：确认本机已安装 Visual Studio 2026 或 2022，并可被 CMake 自动发现；当前脚本会自动做 preset 回退。
